...
def combine_videos_grid1x3(video_paths, output_path, frame_size=(1064//2, 1114//2), fps=15):
    """
...
fps = 15
batch_size = 3
num_batches = (len(thermal_im_video_paths) + len(kaist_video_paths)) // batch_size

# Chunk thermal_im_video_paths and kaist_video_paths into batches of 3 videos alternating between them
video_paths = []
for i in range(num_batches):
    video_paths.append(thermal_im_video_paths[i*batch_size:(i+1)*batch_size])
    video_paths.append(kaist_video_paths[i*batch_size:(i+1)*batch_size])

for i in range(num_batches):
    output_path = f"rgb_thermal_grid_1x3_{i+1}.mp4"
